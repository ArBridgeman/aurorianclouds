...
food_groups_pd = pd.read_csv(food_groups, sep="^", names=["group_id", "group_name"],
                             compression="gzip")

food_groups_pd = clean_entries(food_groups_pd)

food_items_pd = pd.read_csv(food_items, sep="^", names=["item_id", "group_id", "desc_long",
                                                        "desc_short", "common_name",
                                                        "manufacturer_name", "in_survey",
                                                        "refuse_desc", "refuse_pct",
                                                        "sci_name", "n_fac",
                                                        "pro_fac", "fat_fac", "cho_fac"],
                            compression="gzip")

food_items_pd = clean_entries(food_items_pd)

food_items_joined_pd = pd.merge(food_items_pd, food_groups_pd, on="group_id", how="left")
...
feather.write_feather(food_groups_pd, data_base / "food_groups.feather", compression="lz4")
feather.write_feather(food_items_joined_pd, data_base / "food_items.feather", compression="lz4")
